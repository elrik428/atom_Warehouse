...
--SET      @Days     = 4
SET  @Slice    = 1                 /*@Slice should be equal to @ElapsedTime = 2*/
SET  @ElapsedTime   = 1            /*time needs to complete the parameters download in minutes*/
SET  @StartingDate = '2017-12-11'  /*starting date*/
...
print 'Date  ' + cast(@curdate as varchar (20))

SET @FormatedDTime = substring(rtrim(cast(datepart(yy,@curdate) as char(4))),3,2)+
					 right('0'+rtrim(cast(datepart(mm,@curdate) as char(2))),2)+
					 right('0'+rtrim(cast(datepart(dd,@curdate) as char(2))),2)+
...
--============================****ANTONIS****==================================
                    ...
                    -- set next date & time
						...
						SET  @FormatedDTime =substring(rtrim(cast(datepart(yy,@curdate) as char(4))),3,2) + right('0'+rtrim(cast(datepart(mm,@curdate) as char(2))),2)+ right('0'+rtrim(cast(datepart(dd,@curdate) as char(2))),2)+
                                         right('0'+rtrim(cast(cast(@FrmTime as int)%100 as char(2))),2)
										  --SET  @FormatedDTime =substring(rtrim(cast(datepart(yy,@curdate) as char(4))),3,2) + rtrim(cast(datepart(mm,@curdate) as char(2)))+ rtrim(cast(datepart(dd,@curdate) as char(2)))+
            --                              +rtrim(cast(cast(@FrmTime as int)%100 as char(2)))
                END
                else
                    set @BaseTime = cast(@BaseTime as int)/2401 + cast(@BaseTime as int)%2401
            ...
            END
--============================****ANTONIS****==================================
                set @FormatedDTime = substring(@FormatedDTime,1,6)+
				right('0'+rtrim(cast(cast(@NewBaseTime/100 as int) as char(2))),2)
				 + right('0'+rtrim(cast(cast(@NewBaseTime as int)%100 as char(2))),2) + '00'
                ...
                BEGIN
					...
					SET  @FormatedDTime =substring(rtrim(cast(datepart(yy,@curdate) as char(4))),3,2) + right('0'+rtrim(cast(datepart(mm,@curdate) as char(2))),2)+ right('0'+rtrim(cast(datepart(dd,@curdate) as char(2))),2)+
                                         right('0'+rtrim(cast(cast(@FrmTime as int)%100 as char(2))),2)
                    --SET  @FormatedDTime =substring(rtrim(cast(datepart(yy,@curdate) as char(4))),3,2) + right('0'+rtrim(cast(datepart(mm,@curdate) as char(2))),2)+ right('0'+rtrim(cast(datepart(dd,@curdate) as char(2))),2)+
                    --                      right('0'+rtrim(cast(cast(@FrmTime/100 as int) as char(2))),2)+right('0'+rtrim(cast(cast(@FrmTime as int)%100 as char(2))),2)
                END
				PRINT 'FormatedDTime :'  + @FormatedDTime
				print 'FrmTime  ' + cast(@FrmTime as varchar (20))
				print 'FrmTime  ' + cast(@NewBaseTime as varchar (20))
            -- read next terminal
