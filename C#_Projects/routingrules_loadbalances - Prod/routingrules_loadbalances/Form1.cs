...
namespace routingrules_loadbalances
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            {
                if (String.Compare(FormNew.textBoxB1.Text, "admin") == 0 || String.Compare(FormNew.textBoxB2.Text, "admin") == 0)
                {
                    ConnectionString = "ZAC_Production";
                }
                else if (String.Compare(FormNew.textBoxB1.Text, "admintest") == 0 || String.Compare(FormNew.textBoxB2.Text, "admintest") == 0)
                {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            OdbcCommand delRecsBup = new OdbcCommand("Delete from dbo.MERCHBINS_bup3", conn);
            ...
            OdbcCommand comIns = new OdbcCommand(
                  ...
                  "(select TID, DESTPORT, BINLOWER, BINUPPER, INSTMIN, INSTMAX, GRACEMIN,GRACEMAX, ALLOWED, AMOUNTMIN, AMOUNTMAX from dbo.MERCHBINS)", conn);
            ...
            OdbcCommand comCount_2 = new OdbcCommand("select count(*) from dbo.MERCHBINS_bup3", conn);
            ...
            // Inesrt to backup file
            OdbcCommand comIns_LD = new OdbcCommand(
                  "INSERT INTO dbo.LOADBALANCE_BU(ID,BALANCEGROUP,DESTPORT,LOADTYPE,LOADVALUE,SYNC_ID,GR_VAL,EU_VAL,OTHER_VAL)" +
                  "(SELECT ID,BALANCEGROUP,DESTPORT,LOADTYPE,LOADVALUE,SYNC_ID,GR_VAL,EU_VAL,OTHER_VAL FROM dbo.LOADBALANCE)", conn);
            OdbcDataReader readerIns_LD = comIns_LD.ExecuteReader();
            readerIns_LD.Close();

            // Check records in backup file and original the same
            OdbcCommand comCount_1LD = new OdbcCommand("select count(*) from dbo.LOADBALANCE", conn);
            countMER_LD = Convert.ToInt32(comCount_1LD.ExecuteScalar());

            OdbcCommand comCount_2LD = new OdbcCommand("select count(*) from dbo.LOADBALANCE_BU", conn);
            countBUP_LD = Convert.ToInt32(comCount_2LD.ExecuteScalar());
        ...
        private void cmbGroup_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            listView_MID.CommandText = "select case uploadhostid " +
                                        ...
                                        "when 6 then 'ETHNIKI' " +
                                        "when 202 then 'ALPHA' " +
                                        "when 302 then 'ALPHA_2' " +
                                        "when 205 then 'EUROBANK' " +
                                        "when 7 then 'ATTIKA' " +
                                        "else ' ' " +
                                        "end as Host_descr " +
                                        "from MERCHANTS where mid = ? " +
                                        "and  uploadhostid in ('1','6','202','302','205')  " + 
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            //OdbcCommand com = new OdbcCommand("delete from dbo.MERCHBINS where tid in (select tid from dbo.MERCHANTS where mid =" + comboGroup.SelectedItem + ")", conn);
            if (flag_TID == 0)    // MIDs are chosen for ruling
            {
                OdbcCommand comdel = new OdbcCommand("delete from dbo.MERCHBINS where tid in (select tid from dbo.merchants where mid =" + midDescr.Text + ")", conn);
                OdbcDataReader readerDel = comdel.ExecuteReader();
                readerDel.Close();
            }
            else if (flag_TID > 0)
            {
                OdbcCommand comdel = new OdbcCommand("delete from dbo.MERCHBINS where tid =" + tidDescr.Text, conn);
                OdbcDataReader readerDel = comdel.ExecuteReader();
                readerDel.Close();
            }
                ...
                foreach (ListViewItem itemMID in listView_MIDs.Items)
                {
                    ...
                    if (listBoxOrigin.SelectedItem != null && textBox_NEWBIN.Text == "")
                    {
                        ...
                        if (listBoxOrigin.SelectedItem.ToString() == "KOYBAS")
                        {
                            ...
                            if (listBoxOrigin.SelectedItem.ToString() == "KOYBAS" && countCHK_302 > 0 && textBoxHidden_Route.Text.ToString() == "ALPHABANK_2")
                            {
                                string BankDescr = "ALPHA_2";
                                MessageBox.Show("You cannot choose " + BankDescr + " as KOYBAS as it's not connected to all TIDs for MID " + midCheck + ". " + "\n" + "Please choose another!");
                                //listView1.Clear();
                                //listView1.Columns.Add("Originator");
                                //listView1.Columns.Add("Route");
                        ...
                        if (listBoxOrigin.SelectedItem.ToString() == "AMEXDINERS" && (textBoxHidden_Route.Text.ToString() != "ALPHABANK" && textBoxHidden_Route.Text.ToString() != "ALPHABANK_2"))
                        {
                            MessageBox.Show("AMEX/DINERS is routed to ALPHA Bank ONLY!! " + "\n" + "You can't choose any other Bank");
                            return;
                        }

                        // Check if CUP is routed to PIRAEUS                    090218
                    ...
                    else
                    {
                        ...
                        comCursor_NewBIN_Chk.CommandText = "select top(1) TID,DESTPORT,BINLOWER,INSTMIN,INSTMAX from dbo.MERCHBINS where tid in (select tid from dbo.MERCHANTS where mid = ?) and binlower = '" + textBox_NEWBIN.Text + "'";
            ...
            if (flag_TID == 0)
            {
                ...
                foreach (ListViewItem itemMID in listView_MIDs.Items)
                {
                    ...
                    foreach (ListViewItem item in listView1.Items)
                    {
                        ...
                        if (delFlag == 0 && (chkOriginID == "1" || chkOriginID == "6" || chkOriginID == "202" || chkOriginID == "302" || chkOriginID == "205" || chkOriginID == "7" ||
                        ...
                        if (chkOriginID == "1" || chkOriginID == "6" || chkOriginID == "202" || chkOriginID == "302" || chkOriginID == "205" || chkOriginID == "7")
                        {
                            // Check if trxs will routed to same bank id so to change DESTPORT
                            if (chkOriginID == chkOriginID2)
                            {
                                if (chkOriginID == "202" || chkOriginID == "302")  // Only for Alpha
                                {
                                    comCursor.CommandText = "with merchtid(#mid,#tid,#name) as " +
                                                    " (select mid,TID, uploadhostname " +
                                                    "  from dbo.MERCHANTS a, dbo.uploadhosts b  " +
                                                    "  where mid = ? and " +
                                                    "  a.uploadhostid = ? " +
                                                    ...
                                                    "insert into  dbo.MERCHBINS (TID, DESTPORT, BINLOWER, BINUPPER,INSTMIN, INSTMAX, GRACEMIN, GRACEMAX, ALLOWED, AMOUNTMIN, AMOUNTMAX) " +
                                ...
                                else if (chkOriginID != "6" && chkOriginID != "202" && chkOriginID != "302")
                                {
                                    comCursor.CommandText = "with merchtid(#mid,#tid,#name) as " +
                                                        " (select mid,TID, uploadhostname " +
                                                        "  from dbo.MERCHANTS a, dbo.uploadhosts b  " +
                        ...
                        else if (chkOrigin == "AMEXDINERS" && chkOriginID2 == "202")
                        {
                        ...
                        else if (chkOriginID != "1" && chkOriginID != "6" && chkOriginID != "202" && chkOriginID != "302" && chkOriginID != "205" && chkOriginID != "7" &&
                            chkOrigin != "KOYBAS" && chkOrigin != "MEALSANDMORE" && chkOrigin != "TICKETRESTAURANT" && chkOrigin != "AMEXDINERS")
                        {
                            //comCursor_NewBIN_Chk.CommandText = "select top(1) TID,DESTPORT,BINLOWER,INSTMIN,INSTMAX from dbo.MERCHBINS where tid in (select tid from dbo.MERCHANTS where mid = ?) and binlower = '" + chkOrigin + "'";
                            //comCursor_NewBIN_Chk.Parameters.Add("@mid", OdbcType.VarChar, 16).Value = midDescr.Text;
                            //OdbcDataReader readerCursor6 = comCursor_NewBIN_Chk.ExecuteReader();
                            //while (readerCursor6.Read())
                            //{
                            ...
                            {
                                comCursor_NewInst.CommandText = "delete from dbo.MERCHBINS where tid in (select tid from dbo.MERCHANTS  where mid = ?) and binlower = '" + chkOrigin + "';" +
                                                               "with merchtid(#mid,#tid,#name) as " +
                                                    " (select mid,TID, uploadhostname " +
                                                    "  from dbo.MERCHANTS a, dbo.uploadhosts b  " +
                                                    "  where mid = ? and a.uploadhostid = ? " +
                    ...
                    {
                        OdbcCommand comCount_NotosChk = new OdbcCommand("select count(*) from dbo.MERCHbins where mid = ? and tid not in " +
                                                              "(select tid from dbo.MERCHANTS where mid = ? and uploadhostid = 302) and binlower = '999914'", conn);
                    ...
                    comCursor_UpdGmax.CommandText = "with  fndMaxPBnk (tid_, maxinst,desport) as " +
                                                    "(select tid,max(instmax) max_inst, " +
                                                     ...
                                                     "end des_port " +
                                                     "from dbo.MERCHBINS where tid in(select tid from dbo.MERCHANTS where mid = ?) " +
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            if (flag_TID == 0)
            {
                ...
                //int i = 0;
                string sqlCrtQry = "CREATE TABLE [zacrpt].[dbo].[tmp_LB]  (ID int IDENTITY(1,1) PRIMARY KEY, TID nvarchar(16), uploadhost_id varchar(1)) ";   // 090218
                string sqlDelTmpQry = "DELETE from [zacrpt].[dbo].[tmp_LB] ";          // 090218

                // Check for table [zacrpt].[dbo].[tmp_LB] if exists so to create or not    090218
                bool isExists;
                const string sqlStmnt = @"Select count(*) from [zacrpt].[dbo].[tmp_LB]";
                ...
                foreach (ListViewItem itemMID2 in listView_MIDs.Items)
                {
                    ...
                    comLdBal_Del.CommandText = "delete dbo.LOADBALANCE where balancegroup in (select 'M'+tid from dbo.MERCHANTS where mid = ?)";
                    ...
                    lbInsert_.CommandText  = "delete from [zacrpt].[dbo].[tmp_LB]; " +
                                             "Insert into [zacrpt].[dbo].[tmp_LB](tid, uploadhost_id) " +
                                             "Select distinct('M'+TID), uploadhostid FROM dbo.MERCHANTS WHERE mid = ? ";
                    ...
                    foreach (ListViewItem item in listView2.Items)
                    {
                        ...
                        comLdBal_Cursor.CommandText = "with LBTEmp(mTid, UploadID) as " +
                                                       "(select tid, uploadhost_id from [dbo].[tmp_LB]) " +
                                                        " insert into loadbalance(BALANCEGROUP, DESTPORT, LOADTYPE, LOADVALUE, GR_VAL, EU_VAL, OTHER_VAL) " +
        ...
        { // NOT USED
            ...
            //    // Check records in backup file and original the same
            //    OdbcCommand comCount_1 = new OdbcCommand("select count(*) from dbo.MERCHANTS where mid = ? and tid not in " +
            //                                              "(select tid from MERCHANTS where mid = ? and uploadhostid = 1)", conn);
            ...
            //    OdbcCommand comCount_6 = new OdbcCommand("select count(*) from MERCHANTS where mid = ? and tid not in " +
            //                                               "(select tid from MERCHANTS where mid = ? and uploadhostid = 6)", conn);
            ...
            //    OdbcCommand comCount_202 = new OdbcCommand("select count(*) from MERCHANTS where mid = ? and tid not in " +
            //                                   "(select tid from MERCHANTS where mid = ? and uploadhostid = 202)", conn);
            ...
            //    OdbcCommand comCount_205 = new OdbcCommand("select count(*) from MERCHANTS where mid = ? and tid not in " +
            //                                   "(select tid from MERCHANTS where mid = ? and uploadhostid = 205)", conn);
            ...
            //    OdbcCommand comCount_7 = new OdbcCommand("select count(*) from MERCHANTS where mid = ? and tid not in " +
            //                                   "(select tid from MERCHANTS where mid = ? and uploadhostid = 7)", conn);
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem itemTID in listView_TIDs.Items)
            {
                ...
                else
                {
                    ...
                    comCursor_NewBIN_Chk.CommandText = "select top(1) TID,DESTPORT,BINLOWER,INSTMIN,INSTMAX from dbo.MERCHBINS where tid = ? and binlower = '" + textBox_NEWBIN.Text + "'";
                    //Check if MID was chosen from ComboBox or it was added from TextBox
            ...
            foreach (ListViewItem itemTID in listView_TIDs.Items)
            {
                ...
                foreach (ListViewItem item in listView1.Items)
                {
                        ...
                        chkOrigin != "KOYBAS" && chkOrigin != "MEALSANDMORE" && chkOrigin != "TICKETRESTAURANT" && chkOrigin != "AMEXDINERS")
                    {
                        if (dialogResNew_Inst == DialogResult.Yes)
                        {
                            comCursor_NewInst.CommandText = "delete from dbo.MERCHBINS where tid = ? and binlower = '" + chkOrigin + "';" +
                                                           "with merchtid(#mid,#tid,#name) as " +
                                                " (select top 1 mid,TID, uploadhostname " +
                                                "  from dbo.MERCHANTS a, dbo.uploadhosts b  " +
                                                "  where tid = ? and a.uploadhostid = ? " +
        ...
        private void button19_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem itemTID2 in listView_TIDs.Items)
            {
                ...
                // Delete LB from LoadBal file so to insert new selections   090218
                OdbcCommand comLdBal_Del = conn.CreateCommand();
                comLdBal_Del.CommandText = "delete dbo.LOADBALANCE where balancegroup in (select 'M'+tid from dbo.MERCHANTS where tid = ?)";
                ...
                lbInsert_.CommandText = "delete from [zacrpt].[dbo].[tmp_LB]; " +
                                         "Insert into [zacrpt].[dbo].[tmp_LB](tid, uploadhost_id) " +
                                         "Select distinct('M'+TID), uploadhostid FROM dbo.MERCHANTS WHERE tid = ? ";
