...
//            emailTo = "GR-POS-Developers-Dl@euronetworldwide.com";
            ...
            System.IO.File.Delete(@"C:\Users\lnestoras\Desktop\VERICENTRE.xlsx");
...
//            swra.Start();
            string sqlCrtQry =  "create table  [vc30offline].[vc30].[tmp_Tms]  " +
                                ...
                                "[ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_running]	[varchar](60),     " +
                                "[ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_masked]	[varchar](60),     " +
            ...
            string sqlSelQry = "Select   [TID]                " +
                                ...
                                ",[TIP]                       " +
                                ",[ΣΥΝΔΕΣΗ PINPAD]            " +
                                ",[ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_running]  " +
                                ",[ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_masked]   " +
                                ",[ΜΟΝΤΕΛΟ ΤΕΡΜΑΤΙΚΟΥ]        " +
                                ",[ΚΑΤΑΣΚΕΥΑΣΤΗΣ]             " +
                                ",[LOYALTY]                   " +
                                ",[BIRTH DATE]                " +
            ...
            OleDbDataReader rdrCrt = sqlcmdCrt.ExecuteReader();
            rdrCrt.Close();
            sqlcmdCrt.Dispose();
                }
            else
            {
                ...
                sqlcmdDel.Connection = VCConn;
                sqlcmdDel.CommandTimeout = 0;
                sqlcmdDel.CommandType = CommandType.Text;
                sqlcmdDel.CommandText = sqlDelQry;
                //sqlcmdCrt.CommandText = sqlCrtQry;
                OleDbDataReader rdrDel = sqlcmdDel.ExecuteReader();
                rdrDel.Close();
                sqlcmdDel.Dispose();
...
"x.late_versio as [ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_running],                                                         " +
"vc30.relation.appnm as [ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_masked],                                                    " +
...
"      join  (select e.*,                                                                             " +
              ...
              //" where  a.evdate = q.date_max and a.termid = q.termid and appnm like '%EPOS%')e) x on vc30.relation.TERMID=x.termid         " +
"               )                                                                                     " +
"where substring(cast(vc30.relation.appnm as char(10)),9,1) = ('P') " +
//"and  SUBSTRING(vc30.relation.TERMID,1,2) = '73' " +
"and  vc30.relation.CLUSTERID in ('EPOS_PIRAEUS','EPOS_PIRAEUS_EPP','PIRAEUS')                        " +
"AND RIGHT(vc30.relation.TERMID,1) = '" + lstDigiTID + "'";
//" and( x.evdate = ' ' or  x.evdate < '2017-05-29 15:00:00.000' or x.evdate is null )";    // Used for specific reason
 ...
 " [ΦΟΡΟΚΑΡΤΑ] char(255),[LAST PARAMETER CALL] char(255),[ΣΥΝΔΕΣΗ ΤΑΜΕΙΑΚΗΣ] char(255),[TIP] char(255),[ΣΥΝΔΕΣΗ PINPAD] char(255),[ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_running] char(255),[ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_masked] char(255),[ΜΟΝΤΕΛΟ ΤΕΡΜΑΤΙΚΟΥ] char(255), " +
 " [ΚΑΤΑΣΚΕΥΑΣΤΗΣ] char(255),[LOYALTY] char(255),[BIRTH DATE] char(255) )");
            ...
            while (rdrSelExcel.Read())
            {
               ...
               " [ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_running],[ΕΚΔΟΣΗ ΕΦΑΡΜΟΓΗΣ_masked],[ΜΟΝΤΕΛΟ ΤΕΡΜΑΤΙΚΟΥ],[ΚΑΤΑΣΚΕΥΑΣΤΗΣ],[LOYALTY],[BIRTH DATE] ) " +
               " values ('" + rdrSelExcel.GetValue(0).ToString() + "','" + rdrSelExcel.GetValue(1).ToString() + "','" + rdrSelExcel.GetValue(2).ToString().Replace('\'', ' ') + "','" + rdrSelExcel.GetValue(3).ToString().Replace('\'', ' ') + "','"
                ...
                + rdrSelExcel.GetValue(28).ToString() + "','" + rdrSelExcel.GetValue(29).ToString() + "','" + rdrSelExcel.GetValue(30).ToString() + "')");
