...
//            swra.Start();
            string sqlCrtQry = "create table  [vc30].[vc30].[tmp_Tms] " +
                                "([TID] [varchar](60),                        " +
                                "[MID] [varchar](60),                         " +
            ...
            string sqlDrpQry = "Drop table [vc30].[vc30].[tmp_Tms]";
            //string sqlDelQry = "delete from [vc30].[vc30].[tmp_Tms]";
            ...
            string sqlSelQry = "Select   [TID]                " +
                                ...
                                ",[EASYPAYPOINT]  " +
                                ",[SERIAL NUMBER] " +
                                "from [vc30].[vc30].[tmp_Tms] " +
                                "where [TID] <> '44444444' " ;

            // Check for table [vc30].[vc30].[tmp_Tms] if exists so to create or not
            bool isExists;
            const string sqlStmnt = @"Select count(*) from [vc30].[vc30].[tmp_Tms]";
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                string sqlInsertQry = "insert into [vc30].[vc30].[tmp_Tms]		                            " +
...
"AND RIGHT(vc30.relation.TERMID,1) = '" + lstDigiTID + "'";
                ...
                //   -----        EPP terminals
                string sqlInsertQry_EPP = "insert into [vc30].[vc30].[tmp_Tms]		                            " +
               "select  distinct  cast(vc30.relation.TERMID as varchar(30)) as TID,                                                " +
               "cast(a.[value] as varchar(30)) as MID,                                                                             " +
               "b.[value] as [ΔΙΑΚΡΙΤΙΚΟΣ ΤΙΤΛΟΣ],                                                                                 " +
               "c.[value] as [ΔΙΕΥΘΥΝΣΗ],                                                                                          " +
               ...
               "               )                                                                                     " +
               "where substring(cast(vc30.relation.appnm as char(10)),9,1) = ('P')                                   " +
               "and  vc30.relation.CLUSTERID in ('EPOS_PIRAEUS','EPOS_PIRAEUS_EPP')                        " +
               "AND RIGHT(vc30.relation.TERMID,1) = '" + lstDigiTID + "'";


                // Added  by request from y --> ac    PIR2018522-144215
                string updXtraParmQry =
"update  [vc30].[vc30].[tmp_Tms]                                                                           " +
...
"set EASYPAYPOINT = case EASYPAYPOINT when ' '   then 'N/A' else EASYPAYPOINT end                                 ";
                ...
                // Insert Data EPP
                OleDbCommand sqlcmdEPP = new OleDbCommand();
                sqlcmdEPP.Connection = VCConn;
                sqlcmdEPP.CommandTimeout = 0;
                sqlcmdEPP.CommandType = CommandType.Text;
                sqlcmdEPP.CommandText = sqlInsertQry_EPP;
                
                OleDbDataReader rdrIns_EPP = sqlcmdEPP.ExecuteReader();
                rdrIns_EPP.Close();
                ...
                Console.WriteLine("Import for TIDs ending to " + lstDigiTID + "  in [vc30].[vc30].[tmp_Tms] file");
            }
 ...
 " [ΚΑΤΑΣΚΕΥΑΣΤΗΣ] char(255),[LOYALTY] char(255),[BIRTH DATE] char(255),[MCC] char(255),[ETH_DHCP] char(255),[VISA REFUND] char(255),[MASTERCARD REFUND] char(255),[MAESTRO REFUND] char(255),[EASYPAYPOINT] char(255), [SERIAL NUMBER] char(255) )");
            ...
            while (rdrSelExcel.Read())
            {
               ...
               " values ('" + rdrSelExcel.GetValue(0).ToString() + "','" + rdrSelExcel.GetValue(1).ToString() + "','" + rdrSelExcel.GetValue(2).ToString().Replace('\'', ' ') + "','" + rdrSelExcel.GetValue(3).ToString().Replace('\'', ' ') + "','"
                ...
                + rdrSelExcel.GetValue(36).ToString() + "')");
