...
vc30.relation.LASTPARAM_DLD_DATE as [LAST PARAMETER CALL],
...
from (vc30.relation
      ...
      join  (select e.*,
                  ...
                  from
					...
					RIGHT(vc30.termlog.TERMID,1) = '0' and message = 'Download Successful' and status = 'S'
                    group by termid) q
                  where  a.evdate = q.date_max and a.termid = q.termid and appnm like '%PIRA%')e) x on vc30.relation.TERMID=x.termid
      )


where substring(vc30.relation.appnm,9,1) = ('P')
and  vc30.relation.CLUSTERID in ('EPOS_PIRAEUS','EPOS_PIRAEUS_EPP','PIRAEUS')
AND RIGHT(vc30.relation.TERMID,1) = '0'
order by vc30.relation.termid
