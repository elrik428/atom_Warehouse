...
from (vc30.relation
      ...
      left join vc30.PARAMETER cu on vc30.relation.TERMID=cu.PARTID and cu.PARNAMELOC = 'CURRENCYCODE'
...
case (substring(e.vers_pir,9,1)) when ',' then substring(e.vers_pir,1,8) + 'P' when 'P' then (e.vers_pir) end as late_versio
 from
 (select a.evdate, a.termid, a.appnm , substring(a.appnm,charindex('TIRA', a.appnm),9) vers_pir
 from vc30.termlog a,(select  max(evdate) date_max, termid  from vc30.termlog where
 --RIGHT(vc30.termlog.TERMID,1) = @lstDigiTIDand
  message = 'Download Successful' and status = 'S'
 group by termid) q
 where  a.evdate = q.date_max and a.termid = q.termid and appnm like '%TIRA%')e) x on vc30.relation.TERMID=x.termid
               )
where substring(cast(vc30.relation.appnm as char(10)),9,1) = ('P')
and  vc30.relation.CLUSTERID in ('TIRANA')
