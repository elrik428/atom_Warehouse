...
from (vc30.relation
	...
	left join vc30.PARAMETER ac on vc30.relation.TERMID=m.PARTID and m.PARNAMELOC = 'CARD04'
      ...
      left join  (select e.*, case (substring(e.vers_epos,9,1)) when ',' then substring(e.vers_epos,1,8) + 'P' when 'P' then (e.vers_epos) end as late_versio
		 from
		  (select a.evdate, a.termid, a.appnm , substring(a.appnm,charindex('EPOS', a.appnm),9) vers_epos
		   from vc30.termlog a,(select  max(evdate) date_max, termid  
						from vc30.termlog 
		                        where substring(vc30.termlog.termid,1,3) = '730' 
...
AND substring(vc30.relation.TERMID,1,3) = '730'  
